...
#define MAX_DIM 3
struct kd_node_t{//40 bytes per struct
    double x[MAX_DIM];
...
//    if (!len) return 0;
    ...
    if ((n = find_median(t, t + len, i))) {//If n Exist a value
        i = (i + 1) % dim;//choose x,y,z
        n->left  = make_tree(t, n - t, i, dim);//n-t: how many bytes=nodes*sizeof(node)=nodes*40
        printf("n:%d %d %d\tt:%d %d %d\tn:%d\ttr:%d\tn-t: %d\n",n->x[0],n->x[1],n->x[2],t->x[0],t->x[1],t->x[2],n,t,n-t);
        n->right = make_tree(n + 1, t + len - (n + 1), i, dim);//n+1: next memory slot; t+len=final_position; offset=n+1
    }
    return n;
}
...
void nearest(struct kd_node_t *root, struct kd_node_t *nd, int i, int dim,struct kd_node_t **best, double *best_dist){
    ...
    if (!root) return;//root(node) points to NULL/
    d = dist(root, nd, dim);//root->node's distance
    dx = root->x[i] - nd->x[i];
    dx2 = dx * dx;
    ...
    if (++i >= dim) i = 0;//i++;i%=dim;
 
    nearest(dx > 0 ? root->left : root->right, nd, i, dim, best, best_dist);
    /*
     * If(dx>0) Then root->left Else root->right//dx>0: root at node's right side
     * */
    if (dx2 >= *best_dist) return;//If keep finding only gets longer distance
