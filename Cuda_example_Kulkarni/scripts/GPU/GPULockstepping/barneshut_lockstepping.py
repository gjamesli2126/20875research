...
Tree = ["octree", "kdtree"]
Lockstep = ["gpu_non_lockstep", "gpu_lockstep_bitmap", "hybrid"]
	...
	for tree in Tree:
		...
		for lock in Lockstep:
			...
			for input_file in Inputs:
				...
				stats_file = Temp_Path + app + "_" + tree + "_" + lock + "_depth_" + str(depth) + "_nwarps_" + str(Num_Of_Warps_Per_Block) + "_nblocks_" + str(Num_Of_Blocks) + "_input_" + input_file + ".stats"
				time_file = Time_Path + app + "_" + tree + "_" + lock + "_depth_" + str(depth) + "_nwarps_" + str(Num_Of_Warps_Per_Block) + "_nblocks_" + str(Num_Of_Blocks) + "_input_" + input_file + ".time"
				out_file = Temp_Path + app + "_" + tree + "_" + lock + "_depth_" + str(depth) + "_nwarps_" + str(Num_Of_Warps_Per_Block) + "_nblocks_" + str(Num_Of_Blocks) + "_input_" + input_file + ".out"

				# Generate make command & Make
				make_command = " make clean; make" + Make_Config + str(depth)
				make_command = make_command + " NUM_OF_WARPS_PER_BLOCK=" + str(Num_Of_Warps_Per_Block) + " NUM_OF_BLOCKS=" + str(Num_Of_Blocks)
				try:
				...
				for run in range(0, Run_Times):
					...
					exec_command = Exec_Name + Runtime_Flag + Input_Path + input_file + Num_Points
