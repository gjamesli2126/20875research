...
void read_input(int argc, char **argv, int procRank, int numProcs) {
    ...
    if(argc < 2) {
        fprintf(stderr, "usage: nn [-c] [-v] [-s] <k> <npoints> [<nsearchpoints>] <input_file>\n");
        exit(1);
    }
    
    while((c = getopt(argc, argv, "cv:srw")) != -1) {
        switch(c) {
            case 'c':
                check_flag = 1;
                break;
    ...
    SAFE_MALLOC(nearest_neighbor, sizeof(neighbor)*(endIndex-startIndex)*K);
    for (int i = 0; i < (endIndex-startIndex)*K; i ++) {
	...
	int junk;
		...
		if(i<startIndex)
			continue;
                search_points[k].coord[j] = data;
                //search_points[i*DIM + j] = data;
            }
	    if(i<startIndex)
		continue;
            search_points[k].label = junk;
	    k++;
        }
        fclose(in);
        ...
        for(j=0,i = startIndex; i < endIndex; i++,j++) {
            search_points[j].idx = i;
            search_points[j].label = rand() % DIM;
            for(k = 0; k < DIM; k++) {
                search_points[j].coord[k] = 1.0 + (float)rand() / RAND_MAX;
            }
        }
    }
}
...
static int cmpfn_float(const void *a, const void *b) {
    ...
    if (pa->coord[0] < pb->coord[0])
        return -1;
    else if (pa->coord[0] == pb->coord[0])
        return 0;
    else
        return 1;
}

void print_result(int procRank, int numProcs) {
	int numPoints = nsearchpoints;
	int startIndex = procRank * (numPoints/numProcs);
	int endIndex = (procRank == (numProcs - 1))?numPoints:((procRank+1) * (numPoints/numProcs));
	if(numPoints < numProcs)
    ...
    int correct = 0;
    for (int n=0,i = startIndex; i < endIndex; i++,n++) {
        
#ifdef TRACK_TRAVERSALS
	num_nodes_traversed += search_points[n].num_nodes_traversed;
#endif
        int cnts[MAX_LABEL];
        memset(&cnts, 0, sizeof(int) * MAX_LABEL);
        int max_label = 0;
        /*for(int j = 0; j < K; j ++) {
            cnts[nearest_neighbor[n * K + j].point->label] ++;
        }
        ...
        if (search_points[n].label == max_label) {
            correct ++;
        }
    }
    if(numProcs == 1)
