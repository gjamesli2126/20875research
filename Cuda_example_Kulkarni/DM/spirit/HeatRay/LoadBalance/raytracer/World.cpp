...
void World::init (int argc, char **argv)
{
	...
	m_tree = GAL::GAL_GetInstance(pg, new CommLineOpts(argc,argv));
	configure (argc, argv);
...
void World::configure (int argc, char **argv)
{
	...
	//m_tree.build (m_mesh);
	m_tree->GAL_ConstructKDTree(triangles);
...
{
	printf("nodes traversed: %llu\n",m_raytracer.m_kdtree_nodes_traversed);
}
