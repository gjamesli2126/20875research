...
public:
	RandomFloat(uint64_t seed) : generator(seed), uni_dist(0.0f,1.0f), gen(generator,uni_dist), numInvocations(0), sd(seed) { }
	...
	float get() { numInvocations++;return gen(); } ///< Generate a random number
	...
	int getNumInvocations(){return numInvocations;}
	uint64_t getSeed(){return sd;}
};
