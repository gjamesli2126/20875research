...
struct Box
{
	...
	friend std::ostream& operator <<(std::ostream& osobj, const Box& box)
	{
		osobj<<box.min<<std::endl;
		osobj<<box.max<<std::endl;
		osobj<<box.center<<std::endl;
		osobj<<box.half_size<<std::endl;
		return osobj;	
	}
	/**
	  * Operator=.
