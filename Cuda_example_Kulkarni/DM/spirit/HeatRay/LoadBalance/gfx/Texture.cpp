...
size_t Texture::getFIBitmapSize() const
{
#ifdef USE_GL
	FREE_IMAGE_TYPE type = FreeImage_GetImageType(m_data);
	unsigned width       = FreeImage_GetWidth(m_data);
	unsigned height      = FreeImage_GetHeight(m_data);
	unsigned bpp         = FreeImage_GetBPP(m_data);
	
	// check for pixel availability ...
	BOOL header_only = FreeImage_HasPixels(m_data) ? FALSE : TRUE;
...
Texture& Texture::operator = (const Texture& rhs)
{
	if(this != &rhs)
	{
		//texture_name = rhs.texture_name.c_str();
		//texture_name.clear();
		//texture_name.assign(rhs.texture_name.c_str());
		m_ids = rhs.m_ids;
		m_attachments = rhs.m_attachments;
		m_width = rhs.m_width;
