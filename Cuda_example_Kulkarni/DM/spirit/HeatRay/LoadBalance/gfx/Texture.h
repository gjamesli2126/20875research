...
class Texture
{
	public:
		...
		struct Params
		{
			...
			{
				ar & internalFormat & format & envMode & wrapS & wrapT & minFilter & magFilter;
			}
		};

		std::string texture_name;	// Name of this texture.
	...
	private:
		...
		{
			bool isNull = (m_data == NULL);
			ar & isNull;
			if (!isNull) 
			{
				std::string s((const char *)(m_data));
				ar & s;
			}
			ar & m_params & m_height & m_width & m_ids & m_attachments & texture_name;
		}
		...
		void load(Archive& ar, const unsigned version) 
		{
			...
			if (!isNull) 
			{
				std::string s;
				ar & s;
				m_data = (FIBITMAP *)(strdup(s.c_str()));
			} 
			else 
			{
				m_data = 0;
