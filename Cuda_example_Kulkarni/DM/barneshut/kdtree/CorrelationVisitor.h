...
public:
	...
	void SetSimulationParams(float dtim, float es, float tl, long int nts) 
	{
		tol=tl;
		ntimesteps=nts;
		dthf = 0.5f * dtim;
	...
	void UpdatePoint(Point& pot, float mass, Vec dr, float drsq, float epssq);
	bool AreSamePoints(const Point& pot, const GAL_Vertex* node);
