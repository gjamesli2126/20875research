...
void CorrelationVisitor::UpdatePoint(Point& pot, float mass, Vec dr, float drsq, float epssq)
{
	drsq+=epssq;
	float idr=(1.0/sqrt(drsq));
	float nphi=(mass*idr);
	float scale=((nphi*idr)*idr);
	pot.acc.x +=(dr.x*scale);
	pot.acc.y +=(dr.y*scale);
	pot.acc.z +=(dr.z*scale);
}

bool CorrelationVisitor::AreSamePoints(const Point& pot, const GAL_Vertex* node)
{
	bool ret  = true;
	if((node->cp.cofm != pot.cofm) || (node->cp.mass != pot.mass) || (node->lp->vel != pot.vel) || (node->lp->acc != pot.acc))
	{
		ret = false;
	}
	return ret;
...

{
		...
		(const_cast<GAL_Vertex*>(node))->pointsVisited += 1;//block.size();
		(const_cast<GAL_Vertex*>(node))->blockOccupancyFactor += block.size()/(double)1000000;
#endif
...
bool CorrelationVisitor::CanCorrelate(const GAL_Vertex* node, GAL_Vertex* nextNodeToVisit, TIndices& leftBlock, TIndices& rightBlock)
{
	...
	{
		Point* piter = points;//.begin();

		int blkIndex = 0;
		TIndices::size_type bsize = block.size(); 
		float ropensq = node->cp.ropen * node->cp.ropen;
		while(blkIndex < bsize)
		{
			int curIndex = block[blkIndex];
			Vec dr;
			dr.x=(node->cp.cofm.x-(piter+ curIndex)->cofm.x);
			dr.y=(node->cp.cofm.y-(piter+ curIndex)->cofm.y);
			dr.z=(node->cp.cofm.z-(piter+ curIndex)->cofm.z);
			...
			{
				drsq+=eps_squared;
				UpdatePoint(*(piter+ curIndex), node->cp.mass, dr, drsq, eps_squared);
				blkIndex++;
				continue;
			}
			if(leafNode) 
			{
				if(!AreSamePoints(*(piter+ curIndex),node))
					UpdatePoint(*(piter+ curIndex), node->cp.mass, dr, drsq, eps_squared);
			}
			else 
			{
				leftBlock.push_back(curIndex);
				takeLeftBranch = true;
#ifdef TRAVERSAL_PROFILE
				(piter+ curIndex)->nodeDepthAtTruncation.push_back(node->level);
#endif
			}
			blkIndex++;
		}
	...
	/*bool leafNode = g[v].leaf;

	if(aLOPCC && !leafNode)
	{
		(blockStackPointListIter->first).push_back(std::make_pair(nextBlock,const_cast<GAL_Vertex*>(node)));
	}*/
#ifdef MESSAGE_AGGREGATION
	GAL_Vertex* node1=const_cast<GAL_Vertex*>(node);
	if(node1->pseudoLeaf && !(node1->pseudoRoot))
	{
		if(node1->isLeftChild)
		{
			if(takeRightBranch || takeLeftBranch)
				(node1->numBlocksVisited)++;
			if(node1->parent->rightChild)
				node1->parent->rightChild->numBlocksVisited = node1->numBlocksVisited; 
		}
		else
		{
			//if(takeRightBranch || takeLeftBranch)
				(node1->numBlocksVisited)--; 
			if(node1->numBlocksVisited < 0)
				node1->numBlocksVisited = 0;
		}
	}
#endif
