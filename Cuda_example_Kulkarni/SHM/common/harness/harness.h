...
public:
	...
	static int set_block_size(int b) { block_size = b; }
	static int get_splice_depth() { return splice_depth; }
	static int get_num_threads() { return numt; }
	static int lock() { pthread_mutex_lock(&h_mutex); }
	static int unlock() { pthread_mutex_unlock(&h_mutex); }
...
private:
	...
	static uint64_t get_elapsed_usec(int i);
