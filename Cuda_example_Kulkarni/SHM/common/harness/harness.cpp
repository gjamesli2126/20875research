...
pthread_mutex_t Harness::h_mutex;
int Harness::numt;
...
uint64_t Harness::get_elapsed_usec(int i) {
...
void Harness::stop_timing(int i) {
	...
	sum_time[i] += get_elapsed_usec(i)/1000;
#ifdef PAPI
...
float get_mean_min_max(int start, int end, long int *runtimes, int *min, int *max) {
	long int runtimes_sum = 0;
	long int runtimes_max = 0;
	long int runtimes_min = 1 << 30;
	int i;
	float runtimes_avg;
...
float get_stability_stdev(int start, int end, long int *runtimes, float mean, float *stdev) {
...
void Harness::init() {
	if(pthread_mutex_init(&h_mutex,NULL))
...
int Harness::run(int argc, char **argv) {
	...
	for (; run < max_runs; run++) {
		...
		printf("[run:%d]", run);
		for (int i = 0; i < num_counters; i++) {
			runtimes[i][run] = sum_time[i];
			printf("[runtime%d:%ld]", i, runtimes[i][run]);
		}
