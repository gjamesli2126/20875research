...
bool autotuneForFirstIteration = true;
...
void Call_CastPhotonsParallel(int start, int end)
{
	rayTracerObj->CastPhotonsBlock(start, end);
}


void Raytracer::CastPhotonsBlock(int start, int end) 
{
	if (autotuneForFirstIteration && Harness::get_block_size() == 0) {
			autotuneForFirstIteration = false;

			_Autotuner _autotuner(end);
			BlockStack *_tuneStack = new BlockStack();
			Block *_tuneBlock = new Block();
			IntermediaryBlock *_tuneInterBlock = new IntermediaryBlock();
			_tuneInterBlock->block = _tuneBlock;

			std::vector<IntersectInfo> infos(Block::get_max_block());
			std::vector<Ray> rays(Block::get_max_block());
			...
			for (int _t = 0; _t < _autotuner.tuneIndexesCnt; _t++) {
				...
				for(int _tt = 0; _tt < _autotuner.sampleSizes[_t]; _tt++) {
					int i = _indexes[_tt];
					castPhoton_prologue((*points)[i], infos[_tt], rays[_tt], _tuneBlock);
					struct IntermediaryState *_interState = _tuneInterBlock->next ();
				}
				_tuneStack ->  get (0) -> block = _tuneBlock;
				castPhoton_mainAutotune(_tuneStack, &_autotuner);
				_tuneBlock ->  recycle ();
				_tuneInterBlock->reset();
				for(int _tt = 0; _tt < _autotuner.sampleSizes[_t]; _tt++) {
					int i = _indexes[_tt];
					struct IntermediaryState *_interState = _tuneInterBlock->next ();
					castPhoton_epilogue((*points)[i], infos[_tt], rays[_tt], new_points);
#ifdef TRACK_TRAVERSALS
					Harness::lock();
					sum += rays[_tt].num_nodes_traversed;
					Harness::unlock();
#endif
				}
			...
			auto_ptr<IntermediaryBlock> _inter_block(new IntermediaryBlock);

			infos.resize(Block::get_max_block());
			rays.resize(Block::get_max_block());
			...
			for (int _start = start; _start < end; _start += Block::get_max_block()) {
				...
				for (int i = 0; i < Block::get_max_block(); i++) {
					infos[i].init();
					rays[i].init();
				}
				...
				for (int i = _start; i < _end; i++) {
					...
					int idx = i - _start;
					//IntermediaryState *_inter_state = _inter_block->next();
					castPhoton_prologue((*points)[i], infos[idx], rays[idx], _block.get());
				}
				//Harness::stop_timing(1);
				//Harness::start_timing(2);
				_stack->get(0)->block = _block.get();
				castPhoton_main(_stack.get());
				_block->recycle();
				...
				for (int i = _start; i < _end; i++) {
					...
					int idx = i - _start;
					//IntermediaryState *_inter_state = _inter_block->next();
					castPhoton_epilogue((*points)[i], infos[idx], rays[idx], new_points);

#ifdef TRACK_TRAVERSALS
					Harness::lock();
					//cout << i << " " << ray.num_nodes_traversed << endl;
					sum += rays[idx].num_nodes_traversed;
					Harness::unlock();
#endif
				}
				//Harness::stop_timing(3);
...
void Raytracer::photonMap_block() {
	...
	if (Harness::get_block_size() > 0) {
		...
		IntermediaryBlock::max_block = Harness::get_block_size();
	}
	...
	while (!points->empty()) {
		...
		if(0<end)
			Harness::parallel_for(Call_CastPhotonsParallel,0, end);
...
void Raytracer::castPhoton (Photon& p, IntersectInfo &info, Ray &ray, std::vector <Photon >* new_points) {
...
void Raytracer::castPhoton_prologue(const Photon& photon, IntersectInfo &info, Ray &ray, Block *_block) {
Harness::lock();
	m_num_cast_photon_calls++;
Harness::unlock();
	...
	ray.origin = photon.position;
	ray.direction = photon.direction;

	// determine if this photon intersects the triangle mesh.
	m_tree->intersect_prologue (ray, info, _block);
}

void Raytracer::castPhoton_main(BlockStack *_stack) {
	m_tree->intersect_main(_stack);
}

void Raytracer::castPhoton_mainAutotune(BlockStack *_stack, _Autotuner *_autotuner) {
	m_tree->intersect_mainAutotune(_stack, _autotuner);
}

void Raytracer::castPhoton_epilogue(const Photon& p, IntersectInfo &info, Ray &ray, std::vector <Photon >* new_points) {
	//m_tree->intersect_epilogue(_inter_state);
	if (ray.intersects) {
		// This photon has intersected the geometry.
		rvec diffuse 		= info.material->diffuse;
		rvec specular 		= info.material->specular;
...
void Raytracer::doTrace_block() {
	...
	Block::set_max_block(Harness::get_block_size());
#ifndef RAY_TRACE
	Harness::start_timing(1);
#else
	Harness::start_timing(0);
	...
	while (!points->empty()) {
		...
		for (int _start = 0; _start < end; _start += Block::get_max_block()) {
			int _end = min(_start + Block::get_max_block(), end);
			...
			for (int i = _start; i < _end; i++) {
				int idx = i - _start;
				IntermediaryState *_inter_state = _inter_block->next();
				castRay_prologue((*points)[i].ray, (*points)[i].payload, infos[idx], _inter_state, _block.get());
			}

			_stack->get(0)->block = _block.get();
			castRay_main(_stack.get());
			_block->recycle();
			...
			for (int i = _start; i < _end; i++) {
				int idx = i - _start;
				IntermediaryState *_inter_state = _inter_block->next();
				castRay_epilogue((*points)[i].ray, (*points)[i].payload, infos[idx], (*points)[i].index, new_points, &diffuse_points, _inter_state);
...
void Raytracer::castRay_prologue(Ray &ray, Payload &p, IntersectInfo &info, IntermediaryState *_inter_state, Block *_block) {
	m_num_cast_ray_calls++;
	_inter_state->castRay_ret = false;
	// Terminate after 10 bounces.
	if (p.num_bounces > 10) {
		_inter_state->castRay_ret = true;
	}
...
void Raytracer::castRay_main(BlockStack *_stack) {
	m_tree->intersect_main(_stack);
}

float Raytracer::castRay_epilogue(Ray &ray, Payload &p, IntersectInfo &info, int index, std::vector <RayPoint >* new_points, std::vector<DiffusePoint>* diffuse_points,
		IntermediaryState *_inter_state) {
	//m_tree->intersect_epilogue(ray, info);
	if (_inter_state->castRay_ret) return 0.0f;
	float time = (float)0;
	if (ray.intersects) {
		time = info.time;
		if (!(info.time < (float)HUGE_VAL)) {
