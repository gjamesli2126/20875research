...
class Quaternion
{
	public:
		...
		typedef math::Matrix<T, 4, 4> QMatrix;
		...
		inline Quaternion<T> operator/ (const Quaternion &rhs) const
		{
			Quaternion<T> tmp;
			tmp.m_angle = (m_angle * rhs.m_angle) + math::dot (m_axis, rhs.m_axis);
			tmp.m_axis  = math::cross (m_axis * -(T)1.0, rhs.m_axis) - (rhs.m_axis * m_angle) + (m_axis * rhs.m_angle);
			return tmp;
		}
        ...
        *********************************************************************************************************/
		...
		{
			Quaternion<T> _yaw   (cosf ((-yaw)   * (T)0.5f), math::vec3f ((T)0.0f, sinf ((-yaw) * (T)0.5f), (T)0.0f));
			Quaternion<T> _pitch (cosf ((-pitch) * (T)0.5f), math::vec3f (sinf ((-pitch) * (T)0.5f), (T)0.0f, (T)0.0f));
			Quaternion<T> _roll  (cosf ((-roll)  * (T)0.5f), math::vec3f ((T)0.0f, (T)0.0f, sinf ((-roll) * (T)0.5f)));
			result = _yaw * _pitch * _roll;
		}

		void output() {
			printf("%f %f %f %f\n", m_angle, m_axis.v[0], m_axis.v[1], m_axis.v[2]);
		}

		void load(std::ifstream& fin) {
			fin >> m_angle >> m_axis.v[0] >> m_axis.v[1] >> m_axis.v[2];
		}
