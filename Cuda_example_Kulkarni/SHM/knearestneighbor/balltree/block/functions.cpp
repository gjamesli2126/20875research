...
void read_input(int argc, char **argv) {
    ...
    while((c = getopt(argc, argv, "cvtsrw:")) != -1) {
        switch(c) {
            case 'c':
                check_flag = 1;
                break;
...
//        if (i == 1307 || i == 1506) {
//            printf("\n%d: %f %f %f --- ", i, search_points[i].coord[0], search_points[i].coord[1], search_points[i].coord[2]);
            printf("%d: ", i);
            for (int j = K-1; j >= 0; j --) {
                if (j == 0) {
                    //printf("%d (%f %f %f %f %f %f %f) (%1.3f), ", nearest_neighbor[i*K + j].point->idx, search_points[i].coord[0], search_points[i].coord[1], search_points[i].coord[2], search_points[i].coord[3], search_points[i].coord[4], search_points[i].coord[5], search_points[i].coord[6], nearest_neighbor[i*K + j].dist);
                    printf("%d (%1.3f), ", nearest_neighbor[i*K + j].point->idx, nearest_neighbor[i*K + j].dist);
                } else {
                    printf("%d (%1.3f), ", nearest_neighbor[i*K + j].point->idx, nearest_neighbor[i*K + j].dist);
                }
            }
        ...
        int max_label = 0;
        for(int j = 0; j < K; j ++) {
            cnts[nearest_neighbor[i * K + j].point->label] ++;
        }
