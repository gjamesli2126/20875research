...
int main(int argc, char **argv) {
	...
	TIME_RESTART(extra);
	long nMatrixSize = npoints * DEPTH_STAT;
	...
	int stream_workload = npoints / nStreams;
	int point_offset = 0;
	int matrix_workload = nMatrixSize / nStreams;
	CUDA_SAFE_CALL( cudaEventRecord(startEvent,0) );
	for (int i = 0; i < nStreams; i ++) {
		pre_compute_force_gpu<<<blocks, tpb, 0, stream[i]>>>(*gpu_root_d, points_d, npoints, eps_squared, half_dtime, 0, d_correlation_matrix, point_offset, point_offset + stream_workload);
...
//		memset(cpu_buffer, 0, bytes);
	...
	TIME_START(kernel);
	compute_force_gpu<<<blocks, tpb>>>(*gpu_root_d, points_d, gpu_buffer, npoints, eps_squared, half_dtime, 0);
   	cudaThreadSynchronize();	 
	e = cudaGetLastError();
	if(e != cudaSuccess) {
...
void check_depth(Node * node, int _depth)
{
	...
	check_depth(node->left, _depth);
	check_depth(node->right, _depth);
}
...
void track(Node * node, int _depth)
{
	...
	if (node->depth >= _depth) {
		...
		for(cluster_iter = DATA->begin(); cluster_iter != DATA->end(); cluster_iter ++) {
			if(h_correlation_matrix[node->pre_id * npoints + *cluster_iter]) {
...
//		printf("pre_id %d, count = %d， DATA->size = %d.\n", node->pre_id, count, DATA->size());
		...
		track(node->left, _depth);
		track(node->right, _depth);
	}
}
