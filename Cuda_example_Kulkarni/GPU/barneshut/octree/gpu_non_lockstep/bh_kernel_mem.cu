...
void allocate_gpu_tree_device(bh_oct_tree_gpu * h_root, bh_oct_tree_gpu ** d_root) {
	...
	CUDA_SAFE_CALL(cudaMalloc(&(root->stk_nodes), sizeof(int)*root->depth*8*THREADS_PER_BLOCK*NUM_THREAD_BLOCKS));
	CUDA_SAFE_CALL(cudaMalloc(&(root->stk_dsq), sizeof(float)*root->depth*8*THREADS_PER_BLOCK*NUM_THREAD_BLOCKS));
	#endif
