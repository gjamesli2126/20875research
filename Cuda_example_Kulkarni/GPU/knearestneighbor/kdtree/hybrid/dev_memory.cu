...
static gpu_tree* gpu_alloc_tree_dev(gpu_tree *h_tree) {
	...
	d_tree->max_depth = h_tree->max_depth;
	...
	CUDA_SAFE_CALL(cudaMalloc(&d_tree->stk_node, sizeof(int)*h_tree->max_depth*2*NUM_OF_THREADS_PER_BLOCK*NUM_OF_BLOCKS));
	CUDA_SAFE_CALL(cudaMalloc(&d_tree->stk_axis_dist, sizeof(float)*h_tree->max_depth*2*NUM_OF_THREADS_PER_BLOCK*NUM_OF_BLOCKS));
