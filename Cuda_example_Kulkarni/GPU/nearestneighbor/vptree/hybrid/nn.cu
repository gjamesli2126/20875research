...
//	TIME_START(pre_kernel);
	...
	for (int i = 0; i < nStreams; i ++) {
		search_pre_kernel<<<blocks, tpb, 0, stream[i]>>> (__the_tree_d, __GPU_point_Point_d, __GPU_point_array_d, __GPU_node_point_d, d_correlation_matrix, point_offset, point_offset + stream_workload, DEPTH_STAT);
...
//	int* cpu_buffer;
	...
	TIME_START(kernel);
	search_kernel<<<blocks, tpb>>> (__the_tree_d, __GPU_point_Point_d, __GPU_point_array_d, __GPU_node_point_d, gpu_buffer);
