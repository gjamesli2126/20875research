...
void k_nearest_search_k1_GPU(node** root,point target,int tree_num,point* point_list){//return nearest_point
    ...
    printf("My current i is _%d________\n", i);
    //for (i=0;i<tree_num;i++){
    if(i<tree_num){
        printf("Wellcom to GPU success!_____1\n");
        printf("Wellcom to GPU success!_____2\n");
        //current = (node*)0; // NOT current's fault!
        printf("Wellcom to GPU success!_____3\n");

        //current=root[i];
        printf("Wellcom to GPU success!_____4\n");
        
        dim_count=0;
        printf("traverse route:\n");
        //print_this_point_woth_gpu(current->data);
        //test_gpu_cancompile();
        printf("Wellcom to GPU success!_____5\n");
        int k = 6;
        
        printf("Wellcom to GPU success!_____5.1\n");
...
//    write_data_to_txt("8points_rand_max144.txt",orgarr);
    ...
    cudaMemcpy(tree_gpu, tree, sizeof(node) * tree_num, cudaMemcpyDeviceToHost);
    target.values[0] = 32;
    target.values[1] = 11;
    target.values[2] = 65;
    cudaMallocManaged(&found, sizeof(node) * tree_num);
